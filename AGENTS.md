...
- The board has no per-transition server mutation. `web-ui` applies board changes locally (`web-ui/src/state/board-state.ts` re-exports the pure mutations from `src/core/task-board-mutations.ts`) and persists the whole board through the single `workspace.saveState` tRPC mutation, debounced. So a server-side reaction to "task moved to column X" has to diff the incoming board against the persisted one - `saveWorkspaceState`'s `onBoardReplaced` hook exists for exactly that, and `getTaskIdsEnteringFinishedColumn` is the first consumer. Don't go looking for a `trashTask` mutation on the server; only the CLI (`src/commands/task.ts`, via `mutateWorkspaceState`) trashes tasks that way.
- Task worktrees are the main disk consumer: a finished task's checkout keeps its build output (35GB of Rust `target/` has been observed) until something calls `deleteTaskWorktree`. That is safe to do, because it captures a patch of uncommitted work first and `ensureTaskWorktreeIfDoesntExist` replays it, and because `git worktree remove` + `fs.rm` unlink mirrored ignored paths rather than following them into a shared main build target. Batch cleanup goes through `src/workspace/delete-task-worktrees.ts`; don't hand-roll a fourth copy.
